...
#        logger.info("Sending ITS packet to SFF UPF-edge(%s) at %s", (self.remote_sff_ip, self.remote_sff_port), self.sendingtime)
    ...
    def datagram_received(self, data, addr):
        signal.alarm(0)
        self.receivingtime = time.time()
#        logger.info("Received ITS packet from UPF: %s at %s", addr, self.receivingtime)
        RTT = self.receivingtime - self.sendingtime
        logger.info("Round Trip Time for ITS Service from RAN to ITS Server: %s Second", RTT)
        logger.debug("Packet dump: %s", binascii.hexlify(data))
        # Decode all the headers
        decode_vxlan(data, self.server_vxlan_values)
        decode_baseheader(data, self.server_base_values)
